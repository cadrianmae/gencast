...
| **W3.5** | T8b | 1-way | Extends `dump_rates_table` with `provider_filter` + `all_models` modes (depends on T8). |
| **W4** | T9, T10 | parallel | Two formatters, two files-of-effect (one new file). |
| **W5** | T11 | 1-way | CLI command — depends on all of T6–T10 + T8b. |
| **W6** | T12 | 1-way | README. |
...
## Task C8b: Extend `dump_rates_table` with provider + all-models filters

**Files:**
- Modify: `gencast/pipeline/estimate.py`
- Modify: `tests/unit/test_estimate.py`

This task extends T8 to expose LiteLLM's full model catalogue (2,706 models across 15+ providers) instead of only the 7 hardcoded bundled-default models. Used by the v1.2 `notebook-init` skill via dynamic context injection — gives the assistant full pricing-universe context when suggesting models.
...
```bash
PYTHONPATH=. ./venv/bin/pytest tests/unit/test_estimate.py -v 2>&1 | grep "FAILED\|test_dump_rates_table"
```

Expected: 5 failures — `dump_rates_table` doesn't accept `provider_filter` or `all_models` kwargs yet.

- [ ] **Step 3: Replace `dump_rates_table` in `gencast/pipeline/estimate.py`**

Locate the existing `dump_rates_table()` function from T8 and replace it with this expanded version:

```python
def dump_rates_table(
    *,
    provider_filter: str | corebricks.None = None,  # see note below
...
**Important typing fix** — the literal `corebricks.None` above is a placeholder. Use `str | None` (the existing project style — see `gencast/pipeline/preflight.py` for the pattern). The actual signature is:
...
```bash
git add gencast/pipeline/estimate.py tests/unit/test_estimate.py
git commit -m "v1.1 T8b: dump_rates_table accepts --provider + --all-models filters (LiteLLM full catalogue)"
```
...
| `--provider` + `--all-models` (LiteLLM full catalogue) | T8b + T11 |
